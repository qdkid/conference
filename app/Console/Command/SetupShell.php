...
class SetupShell extends AppShell {
	...
	public function createDummyClassSchedule () {
		...
		$teachers = $this->Teacher->find ("list", array ("fields"=> array ("Teacher.id","Teacher.last_name")));
	...
	public function createDummyEnrollment () {
		...
		$students = $this->Student->find ("list", array ("fields"=> array ("Student.id","Student.last_name")));
		...
		foreach ($class_schedules as $class) {
				...
				for ($i=0; $i< $max_seats; $i++) {
					$enrollments[] =
					array ("class_schedule_id"=>$class['ClassSchedule']['id'],
